...
class BeamAttack:
    ...
    def _updateBeam(self, player, enemies, camera, renderer):
        ...
        for enemy in enemies:
            ...
            if distToLine(enemyScreenPos["x"], enemyScreenPos["y"], startX, startY, mouseX, mouseY) <= hitRadius               and now - enemy.last_beam_damage_time >= BEAM_DAMAGE_COOLDOWN:
                enemy.take_damage(player.damage * BEAM_DAMAGE_MULTIPLIER)
                enemy.last_beam_damage_time = now
                if enemy.hp <= 0: player.addXP(10)  
