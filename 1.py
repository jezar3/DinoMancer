    ...
    while True:
        for e in pygame.event.get():
            ...
            if e.type == pygame.KEYDOWN and e.key == pygame.K_RETURN:
                return
        # Pulsing alpha for the prompt text
...
while True:
    #state for the msucic
    pygame.mixer.music.stop()
    pygame.mixer.stop()

    startMenu(window, clock)
    #showText(window, clock, "You broke the family rule", sw // 30, 3000)
    #showText(window, clock, "You are not meant to be a mage", sw // 35, 3000)

    # Start pre-boss background music
    ...
    while running:
        ...
        if frameCount >= BOSS_SPAWN_FRAME and necromancer is None:
            ...
            pygame.mixer.music.play(-1)
            # Swap world visuals for boss fight
        ...
        if mouseLocked:
            ...
            # When unlocked, only move camera with right-click
        ...
        #comprare so if it detects a less number than the orig count it will play the sound 
        if len(slimes) + len(ghouls) < prevCount:
            sfx_death.play()

        #for contex its gonna spawn at 200th currently, i might change it in the future
        slimeTimer += 1
        ...
        #collect all to send to the bea,
        ...
        if beam.weaponMode == "beam" and beam.isAttacking:
            if not beam_channel.get_busy():
                beam_channel.play(sfx_beam, loops=-1)#-1 is unlimited
        else:
            beam_channel.stop()
        ...
        # Check win (necromancer defeated)
