...
# Endless XP scaling: thresholds grow logarithmically so upgrades keep coming
# but slow down gradually, never becoming impossible.
def _xp_threshold(level):
    ...
    if level == 4:
        return 700
    # Level 5+: each subsequent level needs base + growth
    base = 700
    for lv in range(5, level + 1):
        # Diminishing growth per level to stay fair at high waves
...
class Player:
    def __init__(self, x, y, speed, use_nmancer=False):
        ...
        self.max_hp = 8          # reduced from 10 — HP is now more precious
        self.hp = self.max_hp
        self.last_hit_time = -1000
        self.hitTime = -1000
        self.pending_levelup = False

        # --- Debuff state (TreeMob) ---
        self.debuff_active = False
        self.debuff_end_time = 0
        self.debuff_immunity_end = 0    # when immunity expires
    ...
    def addXP(self, amount):
        ...
        if self.xp >= threshold:
            self.level += 1
            # Damage scales with diminishing returns past level 5
            if self.level <= 5:
                self.damage = LEVEL_DAMAGE.get(self.level, self.damage)
            else:
                self.damage += 0.15  # small incremental boost
    ...
    def wasd(self, cam_angle=0):
        ...
        # Rooted: movement is fully locked.
